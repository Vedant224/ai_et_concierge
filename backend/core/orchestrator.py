...
class ConversationOrchestrator:
    ...
    def _finalize_recommendations(self, assistant_response: str) -> str:
        ...
        if self.current_persona.is_sufficient_for_matching() and self.recommendations:
            conversational_block = self._format_recommendations_conversational(self.recommendations)
    ...
    def _build_recommendation_prompt(self, products: List[Product]) -> str:
        ...
        if self.current_persona.professional_background:
            persona_parts.append(f"professional background: {self.current_persona.professional_background}")
        if self.current_persona.financial_goals:
            persona_parts.append(f"financial goals: {self.current_persona.financial_goals}")
        ...
        if self.current_persona.interests:
            persona_parts.append(f"interests: {', '.join(self.current_persona.interests)}")

        persona_context = "; ".join(persona_parts) if persona_parts else "limited known persona data"
        product_lines = "\n".join(
        ...
        return (
            "You are preparing final recommendations for an ET investment assistant.\n"
    ...
    def _build_turn_payload(self, assistant_response: str) -> dict[str, Any]:
        ...
        return {
            ...
            "recommendations": self._serialize_recommendations(self.recommendations) or None,
        }
    ...
    def _safe_extract_and_update(self, history: List[Message]) -> Persona:
        ...
        try:
            ...
            logger.exception("Persona extraction/update failed; preserving previous persona state")
    ...
    def process_turn(self, user_message: str) -> dict[str, Any]:
        ...
        try:
            ...
            assistant_response = "I apologize, but I ran into a temporary issue. Please try again."
    ...
    def stream_turn(self, user_message: str) -> Iterator[str]:
        ...
        try:
            ...
            fallback = "I apologize, but I ran into a temporary issue. Please try again."
    ...
    def inject_system_prompt(self) -> str:
        ...
        """
        base_prompt = """
You are a helpful investment advisor assistant for the ET (Exchange Traded) products platform.
Your goal is to understand the user's financial situation, goals, and preferences through conversation.
...
Keep responses concise and ask one or two questions at a time.
...
"""
        ...
        if self.current_persona.professional_background or self.current_persona.interests:
            persona_text = "\n\nCurrent Understanding of User:\n"
            if self.current_persona.professional_background:
                persona_text += f"- Background: {self.current_persona.professional_background}\n"
            if self.current_persona.financial_goals:
                persona_text += f"- Goals: {self.current_persona.financial_goals}\n"
            if self.current_persona.risk_appetite:
                persona_text += f"- Risk Appetite: {self.current_persona.risk_appetite}\n"
            if self.current_persona.interests:
                persona_text += f"- Interests: {', '.join(self.current_persona.interests)}\n"
